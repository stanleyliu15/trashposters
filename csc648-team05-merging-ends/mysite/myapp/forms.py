...
class SearchForm(forms.Form):
    ...
    selection = forms.ChoiceField(widget=forms.Select(
        attrs={
            'class': 'custom-select resize-select bg-light d-none d-sm-block h-100 py-0',
            'autocomplete': 'off',
            'id': 'filter-post-select-menu'
        }
    ), choices=select_choices)
    value = forms.CharField(label='', max_length=50, widget=forms.TextInput(
        attrs={
